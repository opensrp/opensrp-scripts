...
generateEncounterEtl() {
  ...
  /usr/bin/mysql -u $mysqlUser -p$mysqlPassword  /usr/bin/mysql -u etlgenerator -pPFmqHEN3NzDVEjYhFq4Yt  < ./facility_encounter_report.sql
  /usr/bin/mysql -u $mysqlUser -p$mysqlPassword  < ./facility_encounter_query.sql | sed  's/\t/,/g' > $sourceFile
